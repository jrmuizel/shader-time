...
RenderTaskData fetch_render_task_data(int index){
    ...
    vec4 texel0 = texelFetchOffset(sRenderTasks, uv, 0, ivec2(0, 0));
    vec4 texel1 = texelFetchOffset(sRenderTasks, uv, 0, ivec2(1, 0));
...
vec4[2]fetch_from_gpu_cache_2_direct(ivec2 address){
    return vec4[2](
                                                      texelFetchOffset(sGpuCache, address, 0, ivec2(0, 0)),
                                                      texelFetchOffset(sGpuCache, address, 0, ivec2(1, 0))
    );
}
...
vec4[2]fetch_from_gpu_cache_2(int address){
    ...
    return vec4[2](
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(0, 0)),
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(1, 0))
    );
}
...
vec4[8]fetch_from_gpu_cache_8(int address){
    ...
    return vec4[8](
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(0, 0)),
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(1, 0)),
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(2, 0)),
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(3, 0)),
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(4, 0)),
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(5, 0)),
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(6, 0)),
                                                 texelFetchOffset(sGpuCache, uv, 0, ivec2(7, 0))
    );
}
...
vec4[3]fetch_from_gpu_cache_3_direct(ivec2 address){
    return vec4[3](
                                                      texelFetchOffset(sGpuCache, address, 0, ivec2(0, 0)),
                                                      texelFetchOffset(sGpuCache, address, 0, ivec2(1, 0)),
                                                      texelFetchOffset(sGpuCache, address, 0, ivec2(2, 0))
    );
}
...
vec4[4]fetch_from_gpu_cache_4_direct(ivec2 address){
    return vec4[4](
                                                      ...
                                                      texelFetchOffset(sGpuCache, address, 0, ivec2(1, 0)),
                                                      texelFetchOffset(sGpuCache, address, 0, ivec2(2, 0)),
                                                      texelFetchOffset(sGpuCache, address, 0, ivec2(3, 0))
    );
}
...
Transform fetch_transform(int id){
    ...
    transform . m[0]= texelFetchOffset(sTransformPalette, uv0, 0, ivec2(0, 0));
    transform . m[1]= texelFetchOffset(sTransformPalette, uv0, 0, ivec2(1, 0));
    transform . m[2]= texelFetchOffset(sTransformPalette, uv0, 0, ivec2(2, 0));
    transform . m[3]= texelFetchOffset(sTransformPalette, uv0, 0, ivec2(3, 0));

    transform . inv_m[0]= texelFetchOffset(sTransformPalette, uv0, 0, ivec2(4, 0));
    transform . inv_m[1]= texelFetchOffset(sTransformPalette, uv0, 0, ivec2(5, 0));
    transform . inv_m[2]= texelFetchOffset(sTransformPalette, uv0, 0, ivec2(6, 0));
    transform . inv_m[3]= texelFetchOffset(sTransformPalette, uv0, 0, ivec2(7, 0));
...
PrimitiveHeader fetch_prim_header(int index){
    ...
    vec4 local_rect = texelFetchOffset(sPrimitiveHeadersF, uv_f, 0, ivec2(0, 0));
    vec4 local_clip_rect = texelFetchOffset(sPrimitiveHeadersF, uv_f, 0, ivec2(1, 0));
    ...
    ivec4 data0 = texelFetchOffset(sPrimitiveHeadersI, uv_i, 0, ivec2(0, 0));
    ivec4 data1 = texelFetchOffset(sPrimitiveHeadersI, uv_i, 0, ivec2(1, 0));
    ph . z = float(data0 . x);
...
Glyph fetch_glyph(int specific_prim_address,
    ...
    vec2 glyph = mix(data . xy, data . zw,
                     vec2(bvec2(uint(glyph_index)% 2U != 0U)));
